 ...
 * @see com.lbr.dao.hibernate.domain.Events
...
public class EventsHome {
	...
	public Events merge(Events detachedInstance) {
		...
		try {
			Events result = (Events) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Events findById(java.lang.Integer id) {
		...
		try {
			Events instance = (Events) sessionFactory.getCurrentSession().get(
					"com.lbr.dao.hibernate.Events", id);
			if (instance == null) {
	...
	public List<Events> findByExample(Events instance) {
		...
		try {
			List<Events> results = (List<Events>) sessionFactory
					.getCurrentSession()
					.createCriteria("com.lbr.dao.hibernate.Events")
