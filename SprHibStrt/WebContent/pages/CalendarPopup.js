...
function CallCalendarMethod(fieldName, anchor, changeEndDateAlso){
	changeEndDate = changeEndDateAlso;
	var elem = document.getElementsByName(fieldName)[0];
	//var elem = document.forms[0].getElementsByName(fieldName)[0];
	//alert(elem);
	//cal18.select(elem, anchor,'yyyy-MM-dd HH:mm'); 
	cal18.select(elem, anchor,'yyyy-MM-dd HH:mm');
...
function CP_tmpReturnFunction(y,m,d) { 
	if (window.CP_targetInput!=null) {
		...
		var appendTime = "";
		if(window.CP_targetInput.name.indexOf('start')!=-1){
			appendTime = " 10:00";
		}
		else if(window.CP_targetInput.name.indexOf('end')!=-1){
			appendTime = " 20:00";
		}
		window.CP_targetInput.value = formatDate(dt,window.CP_dateFormat).substring(0,10) +appendTime;
		if(changeEndDate && window.CP_targetInput.name.indexOf('start')!=-1){
			setElementValue('endDate', getElementValue('startDate').substring(0,10)+" 20:00");
		}
		}
	else {
